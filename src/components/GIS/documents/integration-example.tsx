...
import { GeoMapContainer, loadCsvData } from '../index';
import { 
  Legend, 
  LegendGroup, 
  LegendRow, 
...
function IntegrationExample() {
  ...
  const loadData = async () => {
    try {
      ...
      // Note: loadPointsFromCsv function is not available in this version
      // You would need to implement your own point loading logic
      setPoints([]);
...
function ModularLegendExample() {
  const populationGradient = { min: 0, max: 1000000 };
...
function UtilityFunctionsExample() {
  ...
  // Note: createGradientLegend function is not available in this version
  const gradientConfig = {
    gradient: ['#e0f3f8', '#abd9e9', '#74add1', '#4575b4', '#313695'],
    gradientLabels: { left: 'No Population', right: '1M+ Population' }
  };

  // Create categorical legend using utility
  // Note: createCategoricalLegend function is not available in this version
  const categoricalConfig = {
    colorMap: { red: '#ff0000', green: '#00ff00', blue: '#0000ff' },
    categoryLabels: { red: 'Red Items', green: 'Green Items', blue: 'Blue Items' }
  };
